...
const RESEND_TEST_EMAIL = process.env.NOTIFICATION_TEST_EMAIL || 'sabbadih5@gmail.com'
...
export async function sendProjectNotificationEmail({ to, recipientName, subject, message, actionUrl }) {
  if (!process.env.RESEND_API_KEY) {
    return { success: false, error: 'RESEND_API_KEY is not configured' }
  }

  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'
  const targetEmail = FROM_EMAIL.includes('resend.dev') ? RESEND_TEST_EMAIL : to

  if (!targetEmail) {
    return { success: false, error: 'Recipient email is unavailable' }
  }

  const absoluteActionUrl = actionUrl
    ? new URL(actionUrl, siteUrl).toString()
    : siteUrl
...
<body style="margin:0;padding:0;background:#f8fafc;font-family:Arial,sans-serif;color:#0f172a;">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:32px 16px;background:#f8fafc;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#ffffff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;">
        <tr><td style="padding:24px 32px;background:#4f46e5;color:#ffffff;font-size:22px;font-weight:700;">Career Brain</td></tr>
        <tr><td style="padding:32px;">
          <p style="margin:0 0 16px;font-size:16px;">Hi ${escapeHtml(recipientName || 'there')},</p>
          <p style="margin:0 0 24px;font-size:16px;line-height:1.6;">${escapeHtml(message)}</p>
          <a href="${escapeHtml(absoluteActionUrl)}" style="display:inline-block;padding:12px 20px;background:#4f46e5;color:#ffffff;text-decoration:none;border-radius:8px;font-weight:600;">View project</a>
        </td></tr>
      </table>
    </td></tr>
  </table>
  ...
  try {
    ...
    return { success: true, id: data?.id, deliveredTo: targetEmail }
  } catch (error) {
...
export async function sendJobMatchEmail({ to, userName, jobTitle, company, location, jobType, matchScore, matchedSkills, jobId, applicationUrl }) {
  ...
  const VERIFIED_EMAIL = process.env.NOTIFICATION_TEST_EMAIL || 'sabbadih5@gmail.com'
