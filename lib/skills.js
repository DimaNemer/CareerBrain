...
export async function syncGlobalSkills(extractedSkills) {
  ...
  if (toInsert.length > 0) {
    ...
    // Re-fetch all skills matching inserted names to ensure we get their IDs regardless of conflict outcome
    ...
    if (refetchedSkills) {
      const updatedMap = {}
      refetchedSkills.forEach(s => {
        updatedMap[s.name.toLowerCase().trim()] = s
      })
      toInsert.forEach(s => {
        const norm = s.name.toLowerCase().trim()
        if (updatedMap[norm] && !resolvedSkills.some(rs => rs.id === updatedMap[norm].id)) {
          resolvedSkills.push(updatedMap[norm])
        }
      })
    }
  }
...
export async function syncUserSkills(userId, resolvedGlobalSkills, originalSkills, source = 'CV') {
  ...
  for (const globalSkill of resolvedGlobalSkills || []) {
    if (!existingSkillIds.has(globalSkill.id) && !addedInBatch.has(globalSkill.id)) {
      addedInBatch.add(globalSkill.id)
  ...
  if (toInsert.length > 0) {
    const { error: insertError } = await supabase
      .from('user_skills')
      .upsert(toInsert, { onConflict: 'user_id, skill_id', ignoreDuplicates: true })
