...
export async function sendProjectNotification({
  ...
  try {
    ...
    if (emailCooldownMinutes > 0) {
      const cooldownStartedAt = new Date(
        Date.now() - emailCooldownMinutes * 60 * 1000
      ).toISOString()

      const { data: recentEmailedNotification, error: cooldownError } =
        await supabase
          .from('notifications')
          .select('id')
      ...
      if (cooldownError) {
        console.error('[Project notification] Email cooldown check failed:', cooldownError.message)
      } else if (recentEmailedNotification) {
        return {
          success: true,
          notificationId: notification.id,
          email: { success: true, skipped: true, reason: 'cooldown' },
        }
      }
    }
